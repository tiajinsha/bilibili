...
// 分类排行榜
const URL_RANKING_REGION =
  "https://api.bilibili.com/x/web-interface/ranking/region?rid={rid}&day={day}";
// 当前分类排行
const URL_RANKING_ARCHIVE =
  "https://api.bilibili.com/archive_rank/getarchiverankbypartion?tid={tid}&pn={p}";
// 视频详情
const URL_VIDEO_DETAIL =
  "https://api.bilibili.com/x/web-interface/view?aid={aid}&bvid=";
// 详情推荐
// const URL_RECOMMEND = "https://comment.bilibili.com/recommendnew,{aid}";
const URL_RECOMMEND =
  "https://api.bilibili.com/x/web-interface/archive/related?aid={aid}&context=";
// 视频播放地址
const URL_PLAY_URL =
  "https://api.bilibili.com/x/player/playurl?cid={cid}&avid={aid}&platform=html5&otype=json&qn=16&type=mp4&html5=1";
// 详情弹幕
// const URL_BARRAGE = "https://comment.bilibili.com/{cid}.xml";
const URL_BARRAGE = "https://api.bilibili.com/x/v1/dm/list.so?oid={cid}";
// 详情回复
const URL_REPLAY =
  "https://api.bilibili.com/x/v2/reply?type=1&sort=2&oid={oid}&pn={p}&nohot=1";
// 用户视频
...
const URL_VIDEO =
  "https://api.bilibili.com/x/space/arc/search?pn={p}&ps={size}&order=click&keyword=&mid={mid}";
// 热搜
...
// 直播首页
const URL_LIVE_INDEX =
  "https://api.live.bilibili.com/room/v2/AppIndex/getAllList?device=phone&platform=ios&scale=3";
// 分类
const URL_LIVE_AREA =
  "https://api.live.bilibili.com/room/v1/AppIndex/getAreas?device=phone&platform=ios&scale=3&build=1000";
// 直播地址
const URL_LIVE_URL =
  "https://api.live.bilibili.com/room/v1/Room/playUrl?cid={roomid}&platform=h5&otype=json&quality=0";
// 礼物
const URL_LIVE_GIFT =
  "https://api.live.bilibili.com/appIndex/getAllItem?scale=1";
// 房间列表
const URL_ROOM_LIST = "https://api.live.bilibili.com/room/v2/Area/getRoomList";
// 房间信息
const URL_ROOM_INFO =
  "https://api.live.bilibili.com/room/v1/Room/get_info?device=phone&platform=ios&scale=3&build=10000&room_id={roomid}";
// 弹幕配置
const URL_DANMMU_CONFIG =
  "https://api.live.bilibili.com/room/v1/Danmu/getConf?room_id={roomid}&platform=h5";

//视频缩略图
const VideoShot = "http://api.bilibili.com/x/player/videoshot?aid={aid}&index=1";
...
//
const fetchVideoShot = (aId) => {
  return fetch(VideoShot.replace("{aid}", aId))
    .then((res) => res.json())
    .then((json) => json);
};
...
const fetchUserVideo = (param) => {
  return fetch(
    URL_VIDEO.replace("{mid}", param.uId)
      .replace("{p}", param.p)
